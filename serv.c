    ...
    while(1){
        ...
        if(n > 0){
            token = strtok(buf, " ");
                ...
                printf("client called get_length!\n");
            }
                ...
                printf("client called add_front!\n");
						}
            else if (strcmp(token,"add_position") == 0){
                token = strtok(NULL, " ");  // get next token (index)
                idx = atoi(token);
                token = strtok(NULL, " ");  // get next token (value)

                //Make a Call
                if (idx > list_length(mylist)){
                  sprintf(sbuf, "%s%d", ACK, val);                  
                }
                else{
                  val = atoi(token);
